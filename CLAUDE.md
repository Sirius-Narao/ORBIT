...
- **Known, intentional limitations of v0.1**: no broadcasting support in backward rules (shapes are assumed compatible); this is documented, not a bug to silently "fix" by adding broadcasting logic unless asked.
...
- Don't add broadcasting, shape-inference magic, or other generalizations beyond what the current op needs unless asked — v0.1 intentionally assumes compatible shapes.
